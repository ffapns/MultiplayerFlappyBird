namespace Flappy {
    ...
    export class ScoreBoard extends Phaser.Group {
        ...
        public show(score: number): void {
            ...
            const gameOverTween = this.game.add.tween(this.gameOver).to({ alpha: 1, y: Global.Constants.gameHeight / 2 - 100 }, 500, Phaser.Easing.Exponential.Out, true, 500);
            gameOverTween.onStart.add(() => {
                this.wooshSound.play();
            });

            this.scoreWindow.y = Global.Constants.gameHeight / 2 + 20;
            const scoreBoardTween = this.game.add.tween(this.scoreWindow).to({ alpha: 1, y: Global.Constants.gameHeight / 2 }, 500, Phaser.Easing.Exponential.Out, true, 1500);
            scoreBoardTween.onStart.add(() => {
                this.wooshSound.play();
            });

            this.replayButton.y = Global.Constants.gameHeight / 2 + 90;
            const replayButtonTween = this.game.add.tween(this.replayButton).to({ alpha: 1, y: Global.Constants.gameHeight / 2 + 70 }, 500, Phaser.Easing.Exponential.Out, true, 1500);
